    ...
    def parse_document(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for item, _level in doc.iterate_items():
                ...
                item_text = item.export_to_markdown() if hasattr(item, 'export_to_markdown') else str(item)
            ...
            logger.info(f"Docling extracted {len(pages)} pages from {Path(file_path).name}")
            return pages
