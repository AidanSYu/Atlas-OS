...
const ATLAS_BACKEND_BASE_URL: &str = "http://127.0.0.1:8000";
const ATLAS_BACKEND_HEALTH_URL: &str = "http://127.0.0.1:8000/api/framework/health";
...
async fn check_backend_health() -> bool {
    match reqwest::get(ATLAS_BACKEND_HEALTH_URL).await {
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                #[cfg(debug_assertions)]
                {
                    ...
                    log::info!("Dev mode: waiting for external Python backend on {}", ATLAS_BACKEND_BASE_URL);
                    log::info!("Expecting framework health at {}", ATLAS_BACKEND_HEALTH_URL);
                    ...
                    if !ready {
                        log::error!(
                            "Backend not found after 60s. Start it with: .\\scripts\\dev\\run_backend.ps1 and verify {}",
                            ATLAS_BACKEND_HEALTH_URL
                        );
                    }
